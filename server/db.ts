...
export async function listStoredFilesForUser(ownerId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
...
export async function getStoredFileForUser(id: number, ownerId: number) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");

  const result = await db.select().from(storedFiles).where(eq(storedFiles.id, id)).limit(1);
  const file = result[0];
  return file?.ownerId === ownerId ? file : undefined;
}
